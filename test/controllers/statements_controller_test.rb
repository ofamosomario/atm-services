...
class StatementsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create statement' do
    assert_difference('Statement.count') do
      post statements_url, params: { statement: { action: @statement.action, user_id: @statement.user_id } }
    end
  ...
  test 'should update statement' do
    patch statement_url(@statement), params: { statement: { action: @statement.action, user_id: @statement.user_id } }
